...
public class PingPong extends Application {
    ...
    private Consumer<Integer> scoreCallbackP1;
    private Consumer<Integer> scoreCallbackP2;

    public void setScoreCallbackP1(Consumer<Integer> callback) {
        this.scoreCallbackP1 = callback;
    }

    public void setScoreCallbackP2(Consumer<Integer> callback) {
        this.scoreCallbackP2 = callback;
    }
    ...
    private void runGame(GraphicsContext graphicsContext, Canvas canvas) {
        ...
        if (ballXPos < playerOneXPos - PLAYER_WIDTH) {
            updateScores(2); // Player 2 scores
            gameStarted = false;
            resetBall();
            checkWinner();
        }
        ...
        if (ballXPos > playerTwoXPos + PLAYER_WIDTH) {
            updateScores(1); // Player 1 scores
            gameStarted = false;
            resetBall();
            checkWinner();
        }
    ...
    private void updateScores(int player) {
        if (player == 1) {
            scoresP1 += 10;
            if (scoreCallbackP1 != null) {
                scoreCallbackP1.accept(scoresP1);
            }
        } else if (player == 2) {
            scoresP2 += 10;
            if (scoreCallbackP2 != null) {
                scoreCallbackP2.accept(scoresP2);
            }
        }
    }
