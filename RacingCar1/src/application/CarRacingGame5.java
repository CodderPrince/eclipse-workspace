...
public class CarRacingGame5 extends Application {
    ...
    private void update(long now) {
        ...
        if (upPressed) {
            playerY -= playerSpeed;
            if (playerY < 0) playerY = 0; // Prevent moving off the top of the screen
        }
    ...
    /*private boolean checkCollision(Obstacle obstacle) {
        // Check if any part of the player's car overlaps with the obstacle
        return playerX < obstacle.x + CAR_WIDTH &&   // Player's left side is to the left of obstacle's right side
               playerX + CAR_WIDTH > obstacle.x &&   // Player's right side is to the right of obstacle's left side
               playerY < obstacle.y + CAR_HEIGHT &&  // Player's top side is above obstacle's bottom side
               playerY + CAR_HEIGHT > obstacle.y;    // Player's bottom side is below obstacle's top side
    }*/
