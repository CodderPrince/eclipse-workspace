...
public class Game2048 extends Application {
    ...
    public void start(Stage myStage) {
        myStage.setTitle("Game 2048 | PRINCE");

        // Create a FlowPane with padding and alignment
        FlowPane rootNode = new FlowPane();
        rootNode.setStyle("-fx-padding: 50; " +
                "-fx-alignment: center; " +
                "-fx-background-color: linear-gradient(to bottom, #2c3e50, #34495e);");
        rootNode.setVgap(10); // Vertical gap between elements
        ...
        backButton.setStyle("-fx-font-size: 14; -fx-background-color: #f39c12; -fx-text-fill: white; -fx-font-weight: bold;");
        ...
        // Ensure the canvas gains focus for key events
        game.setFocusTraversable(true);
        game.setOnMouseClicked(event -> game.requestFocus());
        game.setOnKeyPressed(event -> {
            game.requestFocus(); // Ensure focus is set when a key is pressed
        ...
        // Add the game canvas to the FlowPane
            ...
            public void handle(long now) {
                ...
                for (int y = 0; y < 4; y++) {
                    for (int x = 0; x < 4; x++) {
                        ...
                        if (value != 0) {
                            gc.fillText(String.valueOf(value), xOffset + CELL_SIZE / 2, yOffset + CELL_SIZE / 2 - 2);
                        }
                    }
                }
                ...
                gc.fillText("Score: " + game.score, game.getWidth() / 2, 40); // Properly positioned score

                // Draw the hint if available
                if (game.hint != null) {
                    gc.setFont(Font.font("Verdana", FontWeight.BOLD, 18));
                    gc.setFill(Color.BLUE);
                    gc.setTextAlign(TextAlignment.CENTER);
                    gc.fillText(game.hint, game.getWidth() / 2, game.getHeight() - 30); // Properly positioned hint
                }

                // Handle win or lose conditions
                if (game.win || game.lose) {
                    ...
                    if (game.win) {
                        gc.fillText("You win!", game.getWidth() / 2, game.getHeight() / 2 - 50);
                    } else if (game.lose) {
                        gc.fillText("Game over!", game.getWidth() / 2, game.getHeight() / 2 - 50);
                    }
                    gc.setFont(Font.font("Verdana", FontWeight.NORMAL, 16));
                    gc.fillText("Press ESC to play again", game.getWidth() / 2, game.getHeight() / 2 + 20);
                }
            }
        }.start();
    }
    ...
    private static int offsetCoors(int arg) {
        return arg * (16 + CELL_SIZE) + 20; // Add consistent padding
    }
    ...
    public static class Game extends Canvas {
        ...
        public void showHint() {
            ...
            for (String move : new String[]{"LEFT", "RIGHT", "UP", "DOWN"}) {
                Game copy = cloneGame();
                Cell[] originalState = copy.cells.clone();
                ...
                if (boardChanged && copy.score >= maxScore) { // Check if the move is valid and improves the score
                    maxScore = copy.score;
                    bestMove = move;
                }
            }

            hint = bestMove != null ? "Try moving " + bestMove : "No valid moves!";
        }
