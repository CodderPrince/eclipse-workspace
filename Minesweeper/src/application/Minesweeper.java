...
public class Minesweeper extends Application {
    ...
    private void setupGame(int size, int mines) {
        ...
        runButton.setStyle("-fx-font-size: 18pt; -fx-text-fill: white; -fx-background-color: #007BFF;");
        ...
        topBar.getChildren().addAll(flagCounter, timerLabel, runButton, tooltip);
        ...
        for (int i = 0; i < gridSize; i++) {
            for (int j = 0; j < gridSize; j++) {
                ...
                cell.setStyle("-fx-font-size: 18pt; -fx-background-color: #ADD8E6; -fx-border-color: black; -fx-border-width: 1px;");
                int finalI = i;
                int finalJ = j;
        ...
        if (isFlagging) {
            if (!revealed[x][y]) {
                if (cells[x][y].getText().equals("F")) {
                    cells[x][y].setText("");
                    cells[x][y].setStyle("-fx-background-color: #ADD8E6; -fx-border-color: black;");
                    flagsUsed--;
