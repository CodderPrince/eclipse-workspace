...
public class Ball02 extends Application {
    ...
    private void runGame(GraphicsContext graphicsContext, Canvas canvas) {
        ...
        if (gameStarted) {
            ...
            // Improved computer movement logic
            if (ballYPos < playerTwoYPos + computerHeight / 2) {
                playerTwoYPos -= Math.min(5, playerTwoYPos - ballYPos); // Move up dynamically
            } else if (ballYPos > playerTwoYPos + computerHeight / 2) {
                playerTwoYPos += Math.min(5, ballYPos - (playerTwoYPos + computerHeight / 2)); // Move down dynamically
            }

            // Keep the computer paddle within bounds
            playerTwoYPos = Math.max(0, Math.min(height - computerHeight, playerTwoYPos));

            graphicsContext.setFill(Color.RED); // Ball color
        ...
        if (ballXPos < playerOneXPos - PLAYER_WIDTH) {
            scoresP2 += 10; // Increment by 10 points
            gameStarted = false;
            resetBall();
            checkWinner();
        }
        ...
        if (ballXPos > playerTwoXPos + PLAYER_WIDTH) {
            scoresP1 += 10; // Increment by 10 points
            gameStarted = false;
            resetBall();
            checkWinner();
        }
